...
import static com.example.medicationtracker.BasePassword.base_password;
...
public class Base {
    public static void base(MainActivity Activity) {

        if (!admin_mode && !staff_mode){
            base_password(Activity, () -> base(Activity));
            return;
        }

        wipe(Activity, "Home", () -> base(Activity));

...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        if love.mouse.wasPressed(1) then
            ...
            if mouseX > self.board.x and mouseY > self.board.y and mouseX < self.board.x + 8*32 and mouseY < self.board.y + 8*32 then
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            for j,tile in pairs(match) do
                self.score = self.score + tile.multiplier * 50
            end

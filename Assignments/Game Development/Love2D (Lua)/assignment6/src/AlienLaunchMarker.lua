...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if love.keyboard.wasPressed('space') and not self.contact and not self.split then
            self.split = true
            local velX, velY = self.alien.body:getLinearVelocity()
            local x,y = self.alien.body:getPosition()
            for i = 1,2 do
                alien = Alien(self.world, 'round', x, y, 'Player')
                alien.fixture:setRestitution(0.4)
                alien.body:setAngularDamping(1)
                if i == 1 then
                    alien.body:setLinearVelocity(velX, velY + 75)
                else
                    alien.body:setLinearVelocity(velX, velY - 75)
                end

...
function Selection:update(dt)
    if love.keyboard.wasPressed('up') and not self.stat then
        ...
        gSounds['blip']:stop()
        gSounds['blip']:play()
    elseif love.keyboard.wasPressed('down') and not self.stat then
...
function Selection:render()
    ...
    for i = 1, #self.items do
        ...
        if i == self.currentSelection and not self.stat then
